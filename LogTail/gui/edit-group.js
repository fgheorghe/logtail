...
class EditGroup {
  ...
  init () {
    ...
    this.buttons.save.addEventListener('click', this.saveGroup.bind(this))

    ipcRenderer.on('edit-group-data', (event, group) => {
      this.inputs.name.value = group.name
      this.groupUuid = group.uuid
      for (const file of group.files) {
        this.watchFiles.push(file.uuid)
        const option = document.createElement('option')
        option.text = file.server.name + ':' + file.path
        option.value = file.uuid
  ...
  cancelAddGroup () {
    ipcRenderer.send('cancel-edit-group')
  }
